...
export async function POST(req: NextRequest, { params }: { params: { id: string; did: string } }) {
  const rl = rateLimit(req, { key: "duels.dispute.concede", ...DEFAULT_MUTATION });
  if (rl) return rl;
  ...
  await prisma.dispute.update({ where: { id: dispute.id }, data: { status: "CONCEDED", resolvedAt: new Date() } });
  await prisma.contribution.update({ where: { id: dispute.contributionId }, data: { disputeStatus: "CONCEDED" } });
    ...
    try {
      await prisma.notification.create({
        data: {
          ...
          body: "Your dispute was conceded. Points removed.",
  ...
  await log(r.user.id, "duel.dispute.concede", { entity: "dispute", entityId: dispute.id, req });
  return ok({ ok: true });
}
